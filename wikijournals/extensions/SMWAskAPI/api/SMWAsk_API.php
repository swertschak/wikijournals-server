    ...
    private function resultToItem($r) {
        ...
        foreach ($r as $field) {
            ...
            if ($rowsubject === false) {
                ...
                $item['title'] = $rowsubject->getTitle();
            }
        ...
        if ($rowsubject !== false) { // stuff in the page URI and some category data
            $item['uri'] = $wgServer . $wgScriptPath . '/index.php?title=' . $rowsubject->getTitle();
            $page_cats = smwfGetStore()->getPropertyValues($rowsubject, new SMWDIProperty( '_INST' )); // TODO: set limit to 1 here
            if (count($page_cats) > 0) {
                $this->addPropToItem($item, 'type', array(reset($page_cats)->getTitle()));
            }
        }
